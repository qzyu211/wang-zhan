...
If you notice, the vector is represented vertically, which is an important nuance. Therefore, to visualize the same vector horizontally, the transpose must first be taken. Such a vector can be denoted as $$\vec{x}_1^\top$$. Another way to denote the first observation is $$X_1$$ (upper-case, italicized, Roman letter X, with a subscript 1). Then the vector of $$X_i's$$, $$i=1,2,\cdots ,n$$ can be denoted as $$\vec{X}$$.

Then it also follows that the data matrix $$\textbf{X}$$ can also be represented as follows,
